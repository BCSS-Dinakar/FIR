...
export default function FIRLayout() {
  ...
  const { collapsed, setCollapsed, dark, setDark, officer, setOfficer, loadingUser } = useGlobals();

  useEffect(() => {
    if (!loadingUser && !officer) {
      navigate('/login');
    }
  }, [loadingUser, officer, navigate]);
  ...
  return (
    <div className={`h-screen overflow-hidden flex transition-colors duration-300 ${T.bg(dark)}`}>
      ...
      <aside className={`hidden lg:block border-r shrink-0 relative z-40 h-screen transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'
        } ${T.sidebar(dark)}`}>
        ...
        <button
          ...
          className={`hidden lg:flex absolute top-7 -right-3.5 w-7 h-7 rounded-full border-2 items-center justify-center z-50 shadow-md transition-all duration-300 ${dark
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 z-50 overflow-y-auto overflow-x-hidden transform transition-transform duration-300 ease-in-out lg:hidden ${mobileOpen ? 'translate-x-0' : '-translate-x-full'
        } ${T.sidebar(dark)}`}>
        <Sidebar
          dark={dark}
          collapsed={false}
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 z-10 relative">
        ...
        <main className="flex-1 min-h-0 overflow-y-auto p-6 md:p-8">
          <Outlet context={{ dark, officer, setOfficer }} />
        </main>
      </div>
    </div>
  );
